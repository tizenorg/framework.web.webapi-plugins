...
public:
    ...
    void GatherSimInformation(TapiHandle* handle, json::Object* out);
    void TryReturn();
...
void SimDetailsManager::GatherSimInformation(TapiHandle* handle, json::Object* out)
{
...
void SimDetailsManager::ResetSimHolder(json::Object* out){
    sim_result_obj_ = out;
    to_process_ = 0;
    mcc_ = 0;
    mnc_ = 0;
...
int SystemInfoListeners::GetSensorHandle() {
  if (m_sensor_handle < 0) {
    LOGD("Connecting to sensor");
    ConnectSensor(&m_sensor_handle);
  } else {
    LOGD("Sensor already connected");
  }
  return m_sensor_handle;
}

void SystemInfoListeners::ConnectSensor(int* result) {
  LOGD("Entered");
...
void SystemInfoListeners::DisconnectSensor(int handle_orientation)
{
  if (handle_orientation >= 0) {
    ...
    LOGD("sensord_stop() returned state = %d", state);
    state = sensord_disconnect(handle_orientation);
    LOGD("sensord_disconnect() returned state %d", state);
  } else {
    LOGD("sensor already disconnected - no action needed");
  }
}
...
void SystemInfoListeners::RegisterDeviceOrientationListener(const SysteminfoUtilsCallback& callback)
{
    if (nullptr == m_device_orientation_listener) {
      ...
      if (!sensor_ret) {
        LOGE("Failed to register orientation change event listener");
      }
...
void SystemInfoListeners::UnregisterDeviceOrientationListener()
{
    if (nullptr != m_device_orientation_listener) {
        ...
        if (!sensor_ret) {
          LOGE("Failed to unregister orientation change event listener");
        }
...
json::Value SysteminfoUtils::GetPropertyValue(const std::string& property)
{
...
void SysteminfoUtils::ReportBattery(json::Object& out) {
    int value = 0;
    ...
    out.insert(std::make_pair("isCharging", json::Value(0 != value)));
}
//TODO maybe make two functions later onGSourceFunc
void SysteminfoUtils::ReportCpu(json::Object& out) {
    LOGD("enter");
...
void SysteminfoUtils::ReportDisplay(json::Object& out) {
    int screenWidth = 0;
    int screenHeight = 0;
...
static bool FetchIsAutoRotation(bool* result)
{
  LOGD("Entered");
  ...
  if ( 0 == vconf_get_bool(
    ...
    throw UnknownException("VCONFKEY_SETAPPL_AUTO_ROTATE_SCREEN_BOOL check failed");
  }
  return result;
}

static std::string FetchStatus(std::string* result)
{
  LOGD("Entered");
  int rotation = 0;
  ...
  if (ret) {
    ...
    if (data.value_count > 0 ) {
      ...
      LOGD("rotation is: %d", rotation);
    } else {
      LOGE("Failed to get data : the size of array is 0. Default rotation would be returned.");
    }
  } else {
    LOGE("Failed to get data(sensord_get_data). Default rotation would be returned.");
  }
  ...
  switch (rotation) {
    ...
    default:
      ...
      throw UnknownException("Received unexpected data");
  }
  *result = status;
  return *result;
}

void SysteminfoUtils::ReportDeviceOrientation(json::Object& out) {
  bool is_auto_rotation = false;
  std::string status = "";

  out.insert(std::make_pair("isAutoRotation", json::Value(FetchIsAutoRotation(&is_auto_rotation))));

  if (FetchStatus(&status).empty()) {
    return;
  }

  out.insert(std::make_pair("status", json::Value(status)));
}

void SysteminfoUtils::ReportBuild(json::Object& out) {
...
void SysteminfoUtils::ReportLocale(json::Object& out) {
...
void SysteminfoUtils::ReportNetwork(json::Object& out) {
...
void SysteminfoUtils::ReportWifiNetwork(json::Object& out) {
...
void SysteminfoUtils::ReportCellularNetwork(json::Object& out) {
...
void SysteminfoUtils::ReportSim(json::Object& out) {

    sim_mgr.GatherSimInformation(system_info_listeners.GetTapiHandle(), &out);
}

void SysteminfoUtils::ReportPeripheral(json::Object& out) {
    try {
...
static void CreateStorageInfo(const std::string& type, struct statfs& fs, json::Object* out) {
...
void SysteminfoUtils::ReportStorage(json::Object& out) {
    int sdcardState = 0;
    struct statfs fs;
    ...
    json::Object& internal_obj = array.back().get<json::Object>();
    ...
    if (0 == vconf_get_int(VCONFKEY_SYSMAN_MMC_STATUS, &sdcardState)) {
        if (VCONFKEY_SYSMAN_MMC_MOUNTED == sdcardState){
            ...
            array.push_back(json::Value(json::Object()));
            json::Object& external_obj = array.back().get<json::Object>();
...
json::Value SystemInfoDeviceCapability::GetCapability(const std::string& key)
{
    json::Value result = json::Value(json::Object());
    json::Object& result_obj = result.get<json::Object>();
